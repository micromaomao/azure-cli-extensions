...
# Windows containers only get the platform-injected rules that actually apply on
# Windows: managed identity (e.g. IDENTITY_ENDPOINT/IDENTITY_HEADER) and the
# container-restart marker. OPENGCS (TERM) and FABRIC rules are Linux-specific.
...
class UserContainerImage(ContainerImage):
    ...
    def from_json(
        ...
        # Start with the customer environment rules. Windows containers only get the
        # managed-identity and restart rules; OPENGCS/FABRIC rules are Linux-specific.
        if is_linux:
