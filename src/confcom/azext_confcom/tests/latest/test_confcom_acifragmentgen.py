...
@patch("azext_confcom.oras_proxy.manifest_fetch_config", return_value=_SINGLE_MANIFEST_CONFIG)
@patch("azext_confcom.oras_proxy.manifest_fetch", return_value=_SINGLE_MANIFEST_RESPONSE)
...
@patch("azext_confcom.oras_proxy.manifest_fetch", return_value={
    "mediaType": "application/vnd.oci.image.index.v1+json",
    "content": {
        "mediaType": "application/vnd.oci.image.index.v1+json",
        "manifests": [
...
def test_oras_attach_multiarch_error(mock_platforms, mock_run):
    ...
    with pytest.raises(SystemExit):
        oras_attach(signed_fragment=mock_fragment, manifest_tag="myregistry.io/myimage:latest")
